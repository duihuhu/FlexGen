    ...
    def generate(self,
        ...
        for j in range(num_layers):
            for k in range(num_gpu_batches):
                ...
                print(type(self.prefill_cache_home[j][k].data))
                print(type(self.cache_home[j][k].data))
        if self.policy.cpu_cache_compute:
    ...
    def generation_loop_debug_normal(self):
        ...
        for stage in ["prefill", "decoding"]:
            for func in ["load_cache", "store_cache", "compute_layer"]:
                ...
                print(f"{name:22s} (per-batch): {np.mean(costs):.6f} s", f"{name:22s} (sum-batch): {np.sum(costs):.6f} s")
...
def run_flexgen(args):
    ...
    print("sep_layer: ", args.sep_layer)
